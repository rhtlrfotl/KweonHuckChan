...
import '../../worker/notice/notice.dart';
...
class AdNoticePost extends StatefulWidget {
  const AdNoticePost({Key? key}) : super(key: key);

  @override
  State<AdNoticePost> createState() => _AdNoticePostState();
}

class _AdNoticePostState extends State<AdNoticePost> {
  ...
  sendPostRequestWithFile(title, desc, List<File> files) async {
    ...
    await Repository.postRequestWithFile(
        title, desc, files, requestNoticePost.toJson());
    EasyLoading.dismiss();
  }
}
